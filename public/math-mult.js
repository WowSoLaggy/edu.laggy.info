...
// Set window.MULT_NUMBERS before loading: single number [2] or range [2,3,4,5,6,7,8,9]
const BATCH_SIZE = 10;
...
function generateMultExamples() {
    const numbers = window.MULT_NUMBERS || [2];
    const multExamples = [];
    const divExamples = [];
    ...
    const examples = [];
    const numBatches = Math.floor(TOTAL_EXAMPLES / BATCH_SIZE);
    for (let b = 0; b < numBatches; b++) {
        ...
        for (let i = 0; i < BATCH_SIZE - half; i++) {
            let ex;
            do {
...
function renderBatch() {
    ...
    if (start + BATCH_SIZE >= allExamples.length) {
        ...
        moreBtn.onclick = () => {
            allExamples = generateMultExamples();
            batchIndex = 0;
            batchStates = {};
            renderBatch();
...
function checkAnswers() {
    ...
    if (totalAnswered === BATCH_SIZE && correctCount === BATCH_SIZE) showFanfare();
    updateButtonVisibility();
}
